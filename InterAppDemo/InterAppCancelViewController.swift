...
class InterAppCancelViewController: UIViewController {
    ...
    class func instantiate() -> InterAppCancelViewController {
        let vc =
            UIStoryboard(name: "Main", bundle: nil).instantiateViewController(
                withIdentifier: "InterAppCancelViewController"
            ) as! InterAppCancelViewController
        return vc
    }
    ...
    @IBAction func dateFromChanged(_ sender: Any) {
        ...
        let startOfDayDate = calendar.date(
            bySettingHour: 0,
            minute: 0,
            second: 0,
            of: dateFromPicker.date,
    ...
    @IBAction func dateToChanged(_ sender: Any) {
        ...
        let calendar = Calendar.current
        let endOfDayOfDateTo = calendar.date(
            bySettingHour: 23,
            minute: 59,
            second: 59,
            of: dateTo,
    ...
    @IBAction func cancelButtonTapped(_ sender: UIButton) {
        ...
        if let decimalString = refundAmountTextfield.text,
            let decimalAmount = Decimal(string: decimalString, locale: Locale.current)
        {
            refundAmount = decimalAmount
        }
        let canceRequestStringURL: String = createCancelRequest(
            isCapture: sender.tag == 1,
            refundAmount: refundAmount,
            reference: referenceTextField.text,
            orderCode: orderCodeTextField.text,
            shortOrderCode: shortOrderCodeTextField.text,
            dateFrom: dateFromString,
            dateTo: dateToString
        )
        (UIApplication.shared.delegate as? AppDelegate)?.performInterAppRequest(
            request: canceRequestStringURL
        )
    }
    ...
    func createCancelRequest(
        ...
        var cancelActionURL = isCapture ? Constants.capturePreauthlUrlString : Constants.cancelUrlString  // vivapayclient://pay/v1?callback=interapp-callback&merchantKey=SG23323424EXS3&appId=com.vivawallet.InterAppDemo&action=cancel
